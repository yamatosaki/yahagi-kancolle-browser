/// Normalized rule models for the wikiwiki.jp/kancolle equipment bonus dataset.
///
...
class EquipmentRef {
  ...
  const EquipmentRef({this.ids = const [], this.typeIds = const [], this.sourceLabel = ''});
...
class CountTableEffect extends Effect {
  ...
  const CountTableEffect(this.byCount, {this.overflow = 'unresolved'});
  @override
  Map<String, Object?> toJson() => <String, Object?>{
        'mode': 'countTable',
        'byCount': byCount.map((k, v) => MapEntry('$k', v.toJson())),
        'overflow': overflow,
      };
}

/// improvementTable: the bonus at each improvement star threshold (★6..★max).
/// Values are the totals as stated by the wiki at that star level.
class ImprovementTableEffect extends Effect {
  ...
  Map<String, Object?> toJson() => <String, Object?>{
        ...
        'byStar': byStar.map((k, v) => MapEntry('$k', v.toJson())),
      };
}
...
class SourceInfo {
  ...
  const SourceInfo({
    ...
    required this.pageLastModified,
